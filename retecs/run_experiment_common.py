...
from xcsf_er_transfer import XCSF_ER_transfer
...
def get_scenario(name):
    if name == 'incremental':
        ...
        sc = scenarios.IndustrialDatasetScenarioProvider(tcfile='DATA/SMBV1.csv')
    elif name == 'EOX':
        sc = scenarios.IndustrialDatasetScenarioProvider(tcfile='DATA/EOX.csv')
    return sc
...
def exp_run_industrial_datasets(iteration, datasets=['EOX']):
    # old
    # ['paintcontrol', 'iofrol', 'gsdtsr']
    ags = [
        lambda: (XCSF_ER_transfer("failcount", "gsdtsr"), retecs.preprocess_xcs, reward.failcount),
        lambda: (XCSF_ER_transfer("failcount", "iofrol"), retecs.preprocess_xcs, reward.failcount),
        lambda: (XCSF_ER_transfer("failcount", "paintcontrol"), retecs.preprocess_xcs, reward.failcount),
    ...
    # outcomment to only get specific stuff
    reward_funs = {
        ...
        for sc in datasets:
            for (reward_name, reward_fun) in reward_funs.items():
                ...
                rl_learning = retecs.PrioLearning(agent=agent,
                                                  ...
                                                  output_dir=DATA_DIR)
                try:
                    # apply transfer learning if possible
                    save_dest = rl_learning.agent_file
